...
<h5>Formulario para actualizar Visitas</h5>
<hr>
<div class="container">
    <form action="/visitas/update/{{$visita->codigo}}" method="post">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <select id="id_usuario" name="id_usuario" required>
                ...
                @foreach($usuarios as $usuario)
                    <option value="{{ $usuario->codigo }}" {{ $visita->id_usuario == $usuario->codigo ? 'selected' : '' }}>
                        {{ $usuario->Nombre }} <!-- Cambia 'nombre' por el campo adecuado -->
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select id="codigo_visitante" name="codigo_visitante" required>
                ...
                @foreach($visitantes as $visitante)
                    <option value="{{ $visitante->codigo }}" {{ $visita->codigo_visitante == $visitante->codigo ? 'selected' : '' }}>
                        {{ $visitante->nombre }} <!-- Cambia 'nombre' por el campo adecuado -->
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" id="proposito" name="proposito" value="{{$visita->proposito}}" required>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="time" id="hora_entrada" name="hora_entrada" value="{{$visita->hora_entrada}}" required>
        </div>

        <div class="form-group">
            <label for="hora_salida">Hora de Salida</label>
            <input type="time" id="hora_salida" name="hora_salida" value="{{$visita->hora_salida}}" required>
        </div>

        <center><button type="submit">Modificar Visita</button></center>
    </form>
</div>
@endsection
